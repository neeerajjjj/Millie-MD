...
let handler = async (m, { conn, usedPrefix, isPrems }) => {
  ...
  conn.sendB(m.chat, `Congratulations you got *+${isPrems ? prem * db.data.users[m.sender].level : free * db.data.users[m.sender].level}* XP & *+${isPrems ? premlimit * 2 : freelimit * 1 }*Limit of daily claim\n\nThe higher the level, the higher the XP you get`, wm, 0, [[`Balance`, `${usedPrefix}balance`]], m)
